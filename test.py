...
def evaluate(data_name, val_output, test_labels, val, topk=(1, 2, 3, 4, 5,)):
    ...
    for k in topk:
        ...
        print("我的Top"+str(k)+":",np.count_nonzero(correct_k)/batch_size)
        precision_k = 0.0
        # recall_k = 0.0
...
class DiseasesPredictor:
    def __init__(self, feat_data, multi_class_num, labels, adj_lists, feature_dim,
        ...
        self.features.weight = nn.Parameter(
            torch.FloatTensor(feat_data).to(self.device), requires_grad=False)
    ...
    def __train__(self, selected_model, train, labels, loop_num=100, batch_num=512, lr=0.01):
        ...
        for batch in range(loop_num):
            ...
            loss = selected_model.loss(batch_nodes,
                                       Variable(torch.FloatTensor(labels[np.array(batch_nodes, dtype=np.int64)]).to(self.device)),)
            loss.backward()
            optimizer.step()
            end_time = time.time()
