...
def generate_simulated_data(number_per_dis=10,rd_list=None):
    # 获取文件所在目录
    ...
    if rd_list is None:
        rd_list=rd_to_hpo_prob.keys()

    simulated_data = []
    for rd_code in tqdm(rd_list):
...
def generate_pkl_from_source_data_test(init_feature_dim):
    """根据测试集中的疾病生成能覆盖的所有疾病的最小Data对象
    通过test_data文件夹和train_data文件夹中的文件生成PyG的Data对象，
    train_data中的案例不全用，只用test_data出现的疾病对应的样例。
    该对象的结构如下：
    Data(
        x=node_feature_matrix,
        edge_index=edge_index,
    ...
    for hpo_list, rd_list in test_data:
        for rd in rd_list:
            all_the_dis.add(rd)
    num_classes = len(all_the_dis)
    #generate_simulated_data(number_per_dis=100, rd_list=all_the_dis)
    train_data = merge_json_lists_from_folder(os.path.join(current_dir, "train_data"))
    train_data = [[hpo_list, rd_list]  for hpo_list, rd_list in train_data if rd_list[0] in all_the_dis]


    print(num_classes)
    rd_to_index=dict()
    index=0
    for rd in all_the_dis:
        rd_to_index[rd]=index
        index+=1
