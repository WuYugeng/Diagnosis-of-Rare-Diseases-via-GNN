...
"""
CUDA_VISIBLE_DEVICES=5,6,7 torchrun --nproc-per-node=3 run.py
"""
...
class DiseasesPredictor:
    def __init__(self,
        ...
        self.gnn = FSDP(
            ...
            auto_wrap_policy=auto_wrap_policy,  # 自动拆分策略
    ...
    def train(self, epoch=1000, batch_size=200, lr=0.3):
        ...
        train_loader = NeighborLoader(
            ...
            input_nodes = self.data.train_mask[local_rank::world_size],
            shuffle=True,
        ...
        epochs = range(epoch)
        if dist.get_rank() == 0:
            epochs = tqdm(epochs, desc="Training")  # 仅 rank 0 显示进度条
...
if __name__ == '__main__':
    with open('/6TB/wy/wyg/codes/Disease-Prediction-via-GCN-main/Data/pkl_data/graph_data.pkl', 'rb') as f:
        data = pickle.load(f)
    #data.to(torch.device('cuda'))
    model=DiseasesPredictor(
        data=data,
        num_layers=3,
        train_enc_dim=[5000,5000,5000],
        num_neighbors=[5,5,5],
        kernel="SAGE",
        kernel_params=None,
    )
    model.train(epoch=10, batch_size=200, lr=0.3)
    model.test()
